...
def run_pagerank_analysis():
    ...
    print("--- INICIANDO ANÁLISE DE PAGERANK ---")
    
    for category in categories:
        graph_path = os.path.join(graphs_dir, f"{category}_graph.json")
        if not os.path.exists(graph_path):
            print(f"Erro: Grafo não encontrado em '{graph_path}'. Pulando...")
            continue
            
        print(f"Carregando o grafo de {category}...")
        ...
        print(f"Calculando PageRank para {category} ({len(id_to_word)} vértices)...")
        pr_scores = calc.calculate(graph, id_to_word)
        results[category] = pr_scores
        
        # Extrai os Top 15 termos usando o Min-Heap personalizado
        top_k = calc.get_top_k(pr_scores, k=15)
        top_results[category] = top_k
        print(f"Top 5 termos para {category}: {[word for word, _ in top_k[:5]]}")
        
    # Salva os resultados das métricas brutas em formato JSON
    metrics_path = os.path.join(metrics_dir, "pagerank_results.json")
    with open(metrics_path, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=4)
    print(f"\nMétricas brutas de PageRank salvas em '{metrics_path}'")
    ...
    print(f"Relatório de análise de PageRank salvo em '{report_path}'")
    print("-------------------------------------")
