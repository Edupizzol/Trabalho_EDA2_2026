...
| :--- | :---: | :---: | :---: |
| **Vocabulário Ativo (Nós)** | 18644 | 9117 | 15690 |
| **Associações Distintas (Arestas)** | 265419 | 96905 | 224272 |
| **Densidade do Grafo** | 0.00153 | 0.00233 | 0.00182 |
| **Coeficiente de Agrupamento Médio** | 0.64682 | 0.58452 | 0.64068 |
...
* **Palavras Compartilhadas (Hubs de Domínio):** uso, loja, dia, compra, americana, aparelho, qualidade, produto, prazo, celular, entrega, problema
...
* ótimo, parabéns, super, satisfeito, perfeito, benefício, recomendo

### Exclusivas de Críticas Negativas (`BAD`):
* diferente, marca, ruim, dinheiro, troca, cliente, pedido, péssimo, peça, devolução, americano, contato, defeito, empresa, resposta, retorno
...
| :--- | :---: | :---: | :---: |
| **problema** | #25 | #8 | Δ 17 posições |
| **americana** | #19 | #3 | Δ 16 posições |
| **loja** | #14 | #4 | Δ 10 posições |
| **dia** | #10 | #2 | Δ 8 posições |
| **uso** | #23 | #16 | Δ 7 posições |
| **compra** | #13 | #6 | Δ 7 posições |
| **qualidade** | #2 | #9 | Δ 7 posições |
| **prazo** | #6 | #11 | Δ 5 posições |
...
1. `excelente` ↔ `produto` (Frequência de Coocorrência: **8569**)
2. `qualidade` ↔ `produto` (Frequência de Coocorrência: **8142**)
3. `produto` ↔ `prazo` (Frequência de Coocorrência: **6361**)

#### Associações Mais Fortes em MID REVIEWS:
1. `produto` ↔ `qualidade` (Frequência de Coocorrência: **880**)
2. `produto` ↔ `prazo` (Frequência de Coocorrência: **820**)
3. `produto` ↔ `entrega` (Frequência de Coocorrência: **780**)

#### Associações Mais Fortes em BAD REVIEWS:
1. `produto` ↔ `dia` (Frequência de Coocorrência: **3472**)
2. `loja` ↔ `americano` (Frequência de Coocorrência: **2604**)
3. `produto` ↔ `entrega` (Frequência de Coocorrência: **2280**)
